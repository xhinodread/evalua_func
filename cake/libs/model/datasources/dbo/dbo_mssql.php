 ...
 */
	function connect() {
		...
		//$this->log("connect() \r\n  ".$config['host']."\r\n password: ".$config['login']."\r\n".$config['password']."\r\n".$config['database'], LOG_DEBUG);
		if (!$config['persistent']) {
			$this->connection = mssql_connect($config['host'] . $port, $config['login'], $config['password'], true);
			/*$this->connection = mssql_connect($config['host'] . $port, $config['login'], $config['password'], true) or die($this->log("connect() \r\n  ".$config['host']."\r\n password: ".$config['login']."\r\n".$config['password']."\r\n".$config['database'].
				"\r\n".' msg: '.print_r(mssql_get_last_message(),1)."\r\n".' Error:'.print_r(error_get_last(),1), LOG_DEBUG));
			*/
			/*
			if(mssql_connect($config['host'] . $port, $config['login'], $config['password'], true)){
				$this->connection = mssql_connect($config['host'] . $port, $config['login'], $config['password'], true);
			}else{
				$this->log("connect() \r\n  ".$config['host']."\r\n password: ".$config['login']."\r\n".$config['password']."\r\n".$config['database'].
				"\r\n".' msg: '.print_r(mssql_get_last_message(),1)."\r\n".' Error:'.print_r(error_get_last(),1), LOG_DEBUG);
