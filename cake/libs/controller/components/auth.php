 ...
 */
	function identify($user = null, $conditions = null) {
		...
		if (is_array($user) && (isset($user[$this->fields['username']]) || isset($user[$model->alias . '.' . $this->fields['username']]))) {
			if (isset($user[$this->fields['username']]) && !empty($user[$this->fields['username']])  && !empty($user[$this->fields['password']])) {
			  ...
			  if(0):
				...
				//$this->log("identify() \n ".$user, LOG_DEBUG);
				///// $validaLdap = $this->loginLDap($user[$model->alias . '.' . $this->fields['username']], $user[$model->alias . '.' . $this->fields['password']]);
				///$this->log("identify() \n validaLdap: ".$validaLdap, LOG_DEBUG);
				
				$username = $user[$model->alias. '.' . $this->fields['username']];
				$password = $user[$model->alias . '.' . $this->fields['password']];
				$validaLdap = $this->loginLDap($username, $password);
		...
		if (!empty($data)) {
			...
			//$validaLdap = true; // CONTINGENCIA, SOLO PARA DESARROLLO *********
			if($validaLdap == true)
				return $data[$model->alias];
			return false;
		}
