	...
	function _createHeader() {
		...
		if ($this->messageId !== false) {
			if ($this->messageId === true) {
				$uiiid = $this->_uuid();
				//$headers['Message-ID'] = '<' . str_replace('-', '', String::uuid()) . '@' . env('HTTP_HOST') . '>'; // LINEA ORIGINAL 
				$headers['Message-ID'] = '<' . str_replace('-', '', $uiiid) . '@' . env('HTTP_HOST') . '>';
				//$headers['Message-ID'] = '<' . str_replace('-', '',"588b5fba-0da8-4155-bb24-0a30c0a8c8be") . '@' . env('HTTP_HOST') . '>';
			} else {
